...
const RedeemView = () => {
    ...
    const handleRedeem = async (e: React.FormEvent) => {
        ...
        try {
            await redeemTokens(coin.address, parseFloat(amount));
            toast({
                title: "Success",
                description: `Redeemed ${amount} ${coin.symbol} successfully!`,
            });
            setAmount('');
        } catch (error) {
            toast({
    ...
    const stablebondReturn = parseFloat(amount || '0') * coin.stablebondRequired;
                        ...
                        <form onSubmit={handleRedeem} className="space-y-6">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-gray-400">
                                    Available balance: {userBalance.toFixed(2)} {coin.symbol}
                                </p>
                            </div>
                            ...
                            <div className="bg-slate-900 rounded-lg p-4 space-y-3">
                                ...
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-400">Stablebond Return</span>
                                    <span className="text-white font-medium">
                                        {stablebondReturn.toFixed(2)} BOND
                                    </span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-400">Exchange Rate</span>
                                    <span className="text-white font-medium">
                                        1 {coin.symbol} = ${coin.price.toFixed(2)} {coin.targetCurrency}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="bg-yellow-900/50 border border-yellow-700 rounded-lg p-4">
                                <div className="flex">
                                    ...
                                    <p className="text-sm text-yellow-100">
                                        Redemption may take up to 24 hours to process during high volume periods.
                                        You'll receive your Stablebonds back once the redemption is processed.
                                    </p>
                                </div>
                            </div>
